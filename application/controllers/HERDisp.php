...
class HERDisp extends MY_Controller {
	public function index()
	{
		...
		$BankResult=$this->general_model->_get(array('table'=>'sp_bank','user_id'=>$user_id));
		if($BankResult){
...
//			$newdate = strtotime ( "+1 day" , $newdate) ;
		...
		$this->load->model('spending');
		$RangResult=$this->spending->Get(array('user_id'=>$user_id,'sortBy'=>'id','sortDirection'=>'desc'),$fmdate,$todate);
		if($optionCategory){
			foreach($optionCategory as $category_id2=>$category) {
				$categoryTotal["$category_id2"] = 0;
				if($action=='yearly') $querySpending=$this->general_model->_get(array('table'=>'sp_monthly_detail','user_id'=>$user_id,'category_id'=>$category_id2,'year'=>$yearly,'sortBy'=>'category_id'));
				else $querySpending=$this->general_model->_get(array('table'=>'sp_monthly_detail','user_id'=>$user_id,'category_id'=>$category_id2,'month'=>$dispmonth,'year'=>$yearly,'sortBy'=>'category_id'));
				if($querySpending){
		...
		$getitem_frequency=$this->general_model->_get(array('table'=>'item_frequency','user_id'=>$user_id,'sortBy'=>'id','sortDirection'=>'desc'));
		if($getitem_frequency){
			foreach($getitem_frequency as $option9) {
				...
				$itemResult9=$this->general_model->_get(array('table'=>'category_item','id'=>$option9->item_id));
				if($itemResult9) {
					$category_item9 = $itemResult9[0]->category_item;
				}		
		...
		$Income = '';
		$Expenese = '';
		$getMonthlyResult=$this->general_model->_get(array('table'=>'sp_monthly','user_id'=>$user_id,'sortBy'=>'reset_date'));
		if($getMonthlyResult){
			foreach($getMonthlyResult as $monthly){
				...
				if($year_month=="$year_month2") $Income .= "<option value='$year_month2' selected>$month_array[$month]/$year=>\$$monthly_income</option>";
				else $Income .= "<option value='$year_month2'>$month_array[$month]/$year=>\$$monthly_income</option>";
				if($year_month=="$year_month2") $Expenese .= "<option value='$year_month2' selected>$month_array[$month]/$year=>\$$monthly_expenese</option>";
				else $Expenese .= "<option value='$year_month2'>$month_array[$month]/$year=>\$$monthly_expenese</option>";
			}
		}
		$balanceResult = $this->general_model->_get(array('table'=>'sp_bank','id'=>$bank_id));
		if($balanceResult) {
			if($balanceResult[0]->balance=='') $balance = 0; else $balance = $e->new_decode($balanceResult[0]->balance);
		}	
		$data['ErrorMessage']=$ErrorMessage;
		$data['optionBank']=$optionBank;
		$data['optionCategory']=$optionCategory;
		$data['optionType']=$optionType;
		$data['optionItem']=$optionItem;
		$data['categoryTotal']=$categoryTotal;
		if(isset($itemTotal)) $data['itemTotal']=$itemTotal;
		$data['balance']=$balance;
		$data['action']=$action;
		if(isset($optionPayFrq)) $data['optionPayFrq']=$optionPayFrq;
		$data['Income']=$Income;
		$data['Expenese']=$Expenese;
		$data['yearly']=$yearly;
		$data['RangResult']=$RangResult;
		$data['cloak_keyword']=$cloak_keyword;
		
		$this->load->view("hERDisp_view",$data);
	}
}
