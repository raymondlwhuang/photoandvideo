...
class Hher extends MainController {
	public function index()
	{
		$this->db->cache_off();
		$this->session->unset_userdata('page');
		if(!$this->session->userdata('id')||!$this->session->userdata('pin'))
		{
			redirect('/', 'refresh');
		}
		...
		if($this->input->post('reminder_id'))
		{
		...
		$ItemResult=$this->general_model->_get(array('table'=>'category_item','sortBy'=>'category_item'));
		if($ItemResult){
		...
		$newdate2 = strtotime ( "-1 month" , strtotime(date('Ymd')));
		$newdate1 = strtotime ( "-7 days" , $newdate2);
		...
		$this->load->view("hher_header",$this->data);
		$this->load->view("hher_view",$this->data);
	}
}
