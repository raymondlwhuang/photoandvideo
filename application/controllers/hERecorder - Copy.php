...
class HERecorder extends MainController {
	public function index()
	{
		...
		$fmdate = date('Y-m')."-01";
		$daysdiff = date('d') + 1;
		$RangResult=$this->spending->Get(array('user_id'=>$this->session->userdata('id')),"$fmdate","$todate");
		...
		$CategoryResult=$this->general_model->_get(array('table'=>'spending_category','user_id'=>0));
		if($CategoryResult){
