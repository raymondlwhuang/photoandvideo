...
$(function () {
	$('#amount').keypad({separator: '|', prompt: '', 
...
//	$("select").change(function(){$('#amount').focus();});
	...
	$("#Save").click(function(event){event.preventDefault();answer=confirm('Are you sure you want to save?');if(answer) Action('Save');$('#keyboard').css('display','none');});
	$("#Logout").click(function(){window.open('Signout',target='_top');});
    setTimeout(function() {$("#category_id").trigger('change');},10);
	$(".close").click(function(){$('#AddDialog').toggle();$('#main,#Result,#reminder_list').toggle();});
	$("#insert,#AddBank,#AddCategory,#AddType,#AddItem,#AddSpender,#AddAutoRec").click(function(){
		$('#AddDialog').toggle();
		$('#main,#Result,#reminder_list').toggle();
...
function SetDialog() {
	...
	if(CallID == 7) {
		...
		else $('#main,#Result,#reminder_list').toggle();
	}
	else  {
...
function Action(action) {
	...
	if(amount != 0 || action != 'Save') {
		 ...
		 if(action == 'Save' && bank_id == 0) $('#ErrorMessage').innerHTML = 'Please choice the Pay From option';
			 ...
			 $('#ErrorMessage').innerHTML = 'Talking to Server.<br/> please wait...';
			 if(action=='Account') var url ="CreditPayOff?category_id="+category_id+"&item_id="+item_id+"&comment_id="+comment_id+"&amount="+amount+"&spender_id="+spender_id+"&type_id="+type_id+"&frequency_id="+frequency_id+"&start_date="+start_date+"&bank_id="+bank_id+"&to_bank="+to_bank+"&year_month="+year_month+"&action="+action+"&yearly="+yearly;
				...
				if(action=='Save'){
					var d = new Date();
					var url ="hher3?category_id="+category_id+"&item_id="+item_id+"&comment_id="+comment_id+"&amount="+amount+"&spender_id="+spender_id+"&type_id="+type_id+"&frequency_id="+frequency_id+"&start_date="+start_date+"&bank_id="+bank_id+"&to_bank="+to_bank+"&year_month="+year_month+"&action=Save&yearly="+yearly;
					$.ajax({
						url: url,
						async: false,
						timeout: 20000,
						success: function() {
							$('#insertline').append("<ul>"+
							"<li>"+d.getDate()+"</li>"+
							"<li>"+$('#item_id option:selected').text()+"</li>"+
							"<li>"+$('#amount').val()+"&nbsp;</li>"+
							"<li>"+$('#type_id option:selected').text()+"</li>"+
							"<li>"+$('#bank_id option:selected').text()+"</li>"
							); 						
						},
						error: function(xhr, textStatus, errorThrown){
					...
					var url ="hher2?category_id="+category_id+"&item_id="+item_id+"&comment_id="+comment_id+"&amount="+amount+"&spender_id="+spender_id+"&type_id="+type_id+"&frequency_id="+frequency_id+"&start_date="+start_date+"&bank_id="+bank_id+"&to_bank="+to_bank+"&year_month="+year_month+"&action="+action+"&yearly="+yearly;
					ajax_load("#Result",url);
				}
				if(type_id==1) {
					if(category_id==1) {
						if(item_id==77) {
							optionBalance[to_bank]=parseFloat(optionBalance[to_bank])+amount;
							optionBalance[bank_id]=parseFloat(optionBalance[bank_id])-amount;
						}
						else optionBalance[bank_id]=parseFloat(optionBalance[bank_id])+amount;
					}
					else optionBalance[bank_id]=parseFloat(optionBalance[bank_id])-amount;
