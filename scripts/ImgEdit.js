...
function PictureFilter() {
	...
	if(flag==1) {
		$('#editimage').html('<img src="ImgFilter?time_x='+new Date().getTime()+'&filename='+filename + '&toNewPic='+toNewPic + '&beforAddText='+beforAddText + '&imgEffect='+imgEffect + '&Smoothvalue='+Smoothvalue + '&red='+red + '&green='+green + '&blue='+blue + '&Contrast='+Contrast + '&Brightness='+Brightness + '&Pixelate='+Pixelate + '"'+'id="image" />');
		$('#editimage').queue(function() {
		  $(this).dequeue();
...
function PictureResize() {
	...
	cur_img="Resizing?time_x="+new Date().getTime()+"&filename="+toNewPic + "&Percent="+Percent + "&toNewPic="+toNewPic2 + '&beforAddText='+beforAddText;
//	alert('tst');
...
function PictureRotation() {
	...
	if(radius==0) {
		...
		cur_img="<img src=\"ImgAngleControl?time_x="+new Date().getTime()+"&filename="+document.getElementById("cur_picture").value + "&degrees="+degrees + "&toNewPic="+toNewPic3 + '&beforAddText='+beforAddText+'"'+" id=\"image\" />";
	}
	else {
		document.getElementById("corner").style.display="block";
		cur_img="<img src=\"ImgRoundCorner?time_x=&filename="+document.getElementById("cur_picture").value + "&degrees="+degrees + "&toNewPic="+toNewPic3 + '&beforAddText='+beforAddText + "&radius="+radius + "&topleft="+topleft + "&bottomleft="+bottomleft + "&bottomright="+bottomright + "&topright="+topright+'"'+" id=\"image\" />";
	}
	$('#editimage').html(cur_img);
	$('#editimage').queue(function() {
	  $(this).dequeue();
...
//	bg_blue=document.getElementById("bg_blue").value;
	...
	if(text_to_display!="" || cur_page==3) {
		...
		{
			cur_img="AddTextToImg?time_x="+new Date().getTime()+"&filename="+beforAddText+"&toNewPic="+toNewPic4 + '&beforAddText='+beforAddText+"&text_to_display="+text_to_display+"&font="+font+"&fontsize="+fontsize+"&textrotate="+textrotate+"&FontR="+FontR+"&FontG="+FontG+"&FontB="+FontB+"&positionx="+positionx+"&positiony="+positiony;
			$('#editimage').html('<img src="'+cur_img + '"'+'id="image" />');
			$('#editimage').queue(function() {
				$(this).dequeue();
...
function ShowOpt(opt) {
	if(opt==1) {
		...
		window.open('EditPicture?thisPicture='+document.getElementById("cur_picture").value+'&org_picture='+document.getElementById("org_picture").value + '&beforAddText='+beforAddText,target='_top');
	}
}
function cropImage() {
	if(document.getElementById("cropbtn").value=="Crop") {
		...
		if(!(x==0&&y==0&&cropWidth==0&&cropHeight==0)) {
			document.getElementById("loading").style.display="inline-block";
			document.getElementById("cropbtn").value="Reset"
			cur_img="Crop?time_x="+new Date().getTime()+"&filename="+document.getElementById("cur_picture").value+"&x="+x+"&y="+y+"&cropWidth="+cropWidth+"&cropHeight="+cropHeight+"&CropPic="+CropPic + '&beforAddText='+beforAddText;
			$('#editimage').html('<img src="'+cur_img + '"'+'id="image" />');
			$('#editimage').queue(function() {
				$(this).dequeue();
			});	
			...
			to5 = window.setTimeout ( function() { window.open('EditPicture?thisPicture='+CropPic+'&org_picture='+document.getElementById("org_picture").value + '&beforAddText='+beforAddText,target='_top'); },   1500  );
		}
	}
	else {
		window.open('EditPicture?thisPicture='+document.getElementById("cur_picture").value+'&org_picture='+document.getElementById("org_picture").value + '&beforAddText='+beforAddText,target='_top');
